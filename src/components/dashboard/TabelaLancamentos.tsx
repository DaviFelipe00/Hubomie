...
interface Lancamento {
  fornecedor: string;
  valor: number;
  pagamento: string; // Garantindo que estamos a usar o campo 'pagamento'
}
...
export function TabelaLancamentos({ lancamentos }: TabelaProps) {
  return (
    <div className="bg-slate-800 p-4 sm:p-6 rounded-xl shadow-lg overflow-x-auto ring-1 ring-white/10">
      <h2 className="text-xl font-bold mb-4 text-white">Contas Pagas no Período</h2>
        ...
        <table className="w-full text-left">
          <thead>
            <tr>
              ...
              <th className="py-3 px-2 sm:px-4 text-[11px] sm:text-xs uppercase tracking-wider font-bold text-slate-400">Data de Pagamento</th>
          ...
          <tbody className="divide-y divide-slate-700">
            {lancamentos.map((lanc, index) => (
              <tr key={index} className="hover:bg-slate-700/50">
                ...
                <td className="py-4 px-2 sm:px-4 text-sm text-slate-400">{lanc.pagamento}</td>
